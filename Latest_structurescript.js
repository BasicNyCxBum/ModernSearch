...
var CONVERSION_THRESHOLD = 10; // (optional)
...
sheet2.getRange('b8').setFormula('=ARRAYFORMULA(IF(RIGHT((substitute(substitute(lower(A8:A), "https://", ""), "http://", "")),1)="/",LEFT(substitute(substitute(lower(A8:A), "https://", ""), "http://", ""), LEN(substitute(substitute(lower(A8:A), "https://", ""), "http://", ""))-1),(substitute(substitute(lower(A8:A), "https://", ""), "http://", ""))))');
      ...
      sheet2.getRange('a1:ae6').setBorder(true, true, true, true, true, true, 'white', SpreadsheetApp.BorderStyle.SOLID);
      sheet2.getRange('b6:ae6').setBorder(true, true, false, true, true, true, 'white', SpreadsheetApp.BorderStyle.SOLID);
      sheet2.getRange('k:k').setBorder(true, null, true, true, true, true, 'white', SpreadsheetApp.BorderStyle.SOLID);
      sheet2.getRange('a7:ae7').setWrap(true);
      sheet2.getRange('b2:b2').setFontWeight('bold');
      sheet2.getRange('b2:b2').setFontSize(14);
      sheet2.getRange('a1:ae7').setFontColor('#666666');
      sheet2.getRange('a6:ae7').setVerticalAlignment('middle');
      sheet2.getRange('a6:ae7').setHorizontalAlignment('center');
    ...
    if (searchcampaignsnumber > 0)
      {
       ...
       var pourcenthighimpression = '';
        ...
        var lowkeywords = AdsApp
           .keywords()
            .withCondition('Status = ENABLED').withCondition('Impressions < 1')
           .forDateRange(periodfromto).get();
...
if (ISlostbudget > 0) { totalcountISlostbudget ++ }
         ...
         if (totalcostaccount > 0)  { shareMANUAL_CPC = parseFloat((costMANUAL_CPC*100 / totalcostaccount).toFixed(1))+'%'  }
          ...
          var reportRSABad = AdsApp.report(
          'SELECT AdGroupId, Status, Impressions, AdStrengthInfo, Cost ' +
          'FROM   AD_PERFORMANCE_REPORT ' +
...
var reportadgroup = AdsApp.report(
          ...
          var idDSA = [];
            while (rows.hasNext())
              ...
              var Impressions = parseFloat(row['Impressions'].replace(/,/g,''));
                ...
                // ExcellentRSA
                if (idRSAExcellent.indexOf(row['AdGroupId']) >= 0 )  { RSAExcellentCost += parseFloat(row['Cost'].replace(/,/g,'')) }
                // GoodRSA
        ...
        var shareRSAExcellentCost = '';
        if (totalcost > 0 )  { shareRSAExcellentCost = parseFloat(RSAGoodCost*100/totalcost).toFixed(1)+'%' }
   ...
   .withCondition('KeywordMatchType = BROAD')
     ...
     .withCondition('Impressions < 10')
       ...
       var reportadgroup = AdsApp.report(
        ...
        var shareRSAExcellentCost = '';
        if (totalcost > 0 )  { shareRSAExcellentCost = parseFloat(RSAExcellentCost*100/totalcost).toFixed(1)+'%' }
